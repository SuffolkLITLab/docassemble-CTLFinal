...
The web scrapper takes simple search criteria, a single party's first name and last name, most likely the user themselves, and searches the CT Judicial Court site. Then, it displays on the next page all cases that have a party with that name and their corresponding docket numbers. For some, that list is short and you can quickly identify your party name. If you are John Smith, we see why the protype is just a demonstration at this point because it returns 50 matches that are hard to read through if you are not familiar with your case. 

his version has limitations, but it begins to build a tool that helps individuals complete the entire processes within our framework. It guides them the same way as instructions would, but a computer does the clicking and follows the instructions behind the scenes. They only have to read the results and make a choice. Rather than read the instructions, follow the instructions accurately, capture the information properly, and bring it back into our framework as directed the computer cuts out steps that are not necessary for the user to do. For this court website it takes 8 steps from opening a new tab to seeing your results to search through. Once the user encounters the tool it takes a single step, the input of the search criteria, to obtain the case information. It makes it simpler. 
